    ...
    public class TitleGatherTest
    {
        ...
        public void Gather()
        {
            ...
            for (int i = 0; i < 2; i++)
            {
                ...
                Assert.HasCount(1, t0.Subs);
                StringAssert.Contains(res.Content, t0.Id.ToString());

                var t00 = t0.Subs[0];
                Assert.AreEqual("1-1", t00.Text);
                Assert.IsNull(t00.Subs);
                StringAssert.Contains(res.Content, t00.Id.ToString());

                var t1 = root[1];
                Assert.AreEqual("2", t1.Text);
                Assert.IsNull(t1.Subs);
                StringAssert.Contains(res.Content, t1.Id.ToString());
            }
        }
    }
}
