...
namespace FCloud3.Utils.Utils
{
    ...
    //{
    //    public static IServiceCollection AddUtils(this IServiceCollection services, SettingsHelper<AppSettingsModel>? settingsAccessor = null)
    //    {
    //        //由于重启前配置都不会重新读取，所以配置读取器应该单例
    //        if(settingsAccessor is not null)
    //            services.AddSingleton<SettingsHelper<AppSettingsModel>>(settingsAccessor);
    //        else
    //            services.AddSingleton<SettingsHelper<AppSettingsModel>>();
    ...
    //        {
    //            var settingsAccessor = ser.GetService<SettingsHelper<AppSettingsModel>>();
